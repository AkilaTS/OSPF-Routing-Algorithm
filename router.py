...
def send_hellos(hello_time):    #are there arguments needed or glabal variables are shared
    ...
    while(True):
        for i in range(len(neighbors)):    # send to each neighbor
           recv_AddressPort = ("127.0.0.1", 10000 + int(neighbors[i]))
           bytesToSend = msg.encode()
           UDPSocket.sendto(bytesToSend, recv_AddressPort)
...
print("Router ",str(node_id)," started")
            ...
            if(seq_no>last_seq_no[sender_id]):
                ...
                for j in range(len(neighbors)):
                    if(j != sender_id):
                        ...
                        UDPSocket.sendto(msg_recd.encode(), ("127.0.0.1", 10000+curr_recv_id))
except KeyboardInterrupt:
	if(debug_term):
